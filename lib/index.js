...
_commander2.default.command('start [script]').description('koahub start script --watch --compile').option('-w, --watch', 'auto restart when a file is modified').option('-c, --compile', 'auto babel process when a file is modified').option('-r, --runtime [dir]', 'Babel compile and start the dir').action(function (script, options) {
    ...
    var binPath = _path2.default.resolve(process.mainModule.filename, '../../');
    var babel = _path2.default.resolve('node_modules/.bin/babel');
        ...
        var _ret = function () {
            var startRuntimeProcess = function startRuntimeProcess(runtimeFile) {
                _nodeNotifier2.default.notify({
                    title: 'KoaHub.js',
                    message: '启动成功',
                    icon: _path2.default.resolve(binPath, 'icons/info.png')
                });
            ...
            // 编译并且监控启动
            if (options.compile) {
                _shelljs2.default.exec(_path2.default.normalize(babel + " " + appName + " --out-dir " + runtimeName));
            }
            ...
            (0, _watch2.default)(appName, runtimeName, function (filePath) {
                var compile = arguments.length <= 1 || arguments[1] === undefined ? true : arguments[1];
                ...
                var timeOut = setTimeout(function () {
                    if (files.length) {
                        for (var key in files) {
                            _shelljs2.default.exec(_path2.default.normalize(babel + " " + files[key].filePath + " --out-file " + files[key].fileRuntimePath));
                        }
                        // 未编译文件清空
                        files = [];
                    }
...
_commander2.default.command('controller [file]').description('koahub create controller').action(function (file) {
    ...
    var srcFile = _path2.default.normalize(_path2.default.resolve(binPath, 'template/controller/index.controller.js'));
