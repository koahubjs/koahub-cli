...
_commander2.default.command('start [script]').description('koahub start script --watch --compile').option('-w, --watch', 'auto restart when a file is modified').option('-c, --compile', 'auto babel process when a file is modified').option('-r, --runtime [dir]', 'Babel compile and start the dir').action(function (script, options) {
    ...
    var app = _path2.default.dirname(script) || _index2.default.app;
    var runtime = options.runtime || _index2.default.runtime;
    ...
    if (options.watch) {
        var _ret = function () {
            ...
            // 编译并且监控启动
            if (options.compile) {
                _shelljs2.default.exec(_path2.default.normalize("./node_modules/.bin/babel " + appName + " --out-dir " + runtimeName));
            }
            ...
            var files = [];
            // 开启文件监控
            (0, _watch2.default)(app, function (filePath) {
                ...
                var timeOut = setTimeout(function () {
                    if (files.length) {
                        for (var key in files) {
                            _shelljs2.default.exec(_path2.default.normalize("./node_modules/.bin/babel " + files[key].filePath + " --out-file " + files[key].fileRuntimePath));
                        }
                        // 未编译文件清空
                        files = [];
                    }
